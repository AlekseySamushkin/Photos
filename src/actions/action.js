...
export const getPhotoInsta = () => dispatch =>  { // функция делает запрос и обновляет store.
   ...
   .then(r=>{
     let per = r.data.map(el=>{
        ...
        per.images.size='small'; // добавляем свойство size, исходя из которого будем выводить фотку в том или ином формате
        return per;
     });
...
// экшен заменяет в store размер фото с нужным id
